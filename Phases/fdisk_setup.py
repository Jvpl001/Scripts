...
def run_fdisk():
    ...
    while True:
        name = input("Enter the installation drive (e.g. sda, nvme0n1, or vda): ").strip().lower()
        # Accept typical Linux disk names: sda/sdb..., nvme0n1/nvme1n1..., or vda (virtio)
        if re.fullmatch(r"sd[a-z]", name) or re.fullmatch(r"nvme\d+n\d+", name) or re.fullmatch(r"vd[a-z]", name):
            break
        else:
    ...
    print("\nPartitioning...")
    # Feed the predefined partitioning script to fdisk via stdin using input_text.
    # This relies on the enhanced run_command that supports input_text.
    ...
    run_command(["mount", "-o", "noatime,compress=zstd,space_cache=v2,subvol=@", part3, "/mnt"])
    run_command(["mkdir", "-p", "/mnt/boot", "/mnt/var", "/mnt/home", "/mnt/.snapshots"])
    run_command(["mount", "-o", "noatime,compress=zstd,space_cache=v2,subvol=@home", part3, "/mnt/home"])
    run_command(["mount", "-o", "noatime,compress=zstd,space_cache=v2,subvol=@var", part3, "/mnt/var"])
    run_command(["mount", "-o", "noatime,compress=zstd,space_cache=v2,subvol=@snapshots", part3, "/mnt/.snapshots"])
